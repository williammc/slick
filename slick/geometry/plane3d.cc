...
namespace slick {
const SlickScalar Plane3d::epsilon = 1.e-30;

std::pair<std::vector<unsigned int>, SlickScalar> Plane3d::fit_plane(
    const std::vector<Eigen::Matrix<SlickScalar, 3, 1> >& points, Plane3d& plane) {
  Eigen::MatrixXd v4_points(points.size(), 4);
  for (std::vector<Eigen::Matrix<SlickScalar, 3, 1> >::size_type i = 0; i < points.size(); i++) {
    v4_points.row(i) = Eigen::Matrix<SlickScalar, 4, 1>(points[i][0], points[i][1], points[i][2], 1);
  }
  ...
  Eigen::Matrix<SlickScalar, 4, 1> v4_plane = sym.eigenvectors().col(sm_id);
...
std::pair<std::vector<unsigned int>, SlickScalar> Plane3d::fit_plane_advanced(
    const std::vector<Eigen::Matrix<SlickScalar, 3, 1> >& points, Plane3d& plane,
    Eigen::Matrix<SlickScalar, 3, 1>* anchor_point1, Eigen::Matrix<SlickScalar, 3, 1>* anchor_point2) {
  using namespace Eigen;
  int npoints = points.size();
  Eigen::Matrix<SlickScalar, 3, 1> v3_bestmean, v3_bestnormal;
  SlickScalar best_distsq=1.e+9;
  int nransac = std::min(100, int(points.size()));
  Eigen::Matrix<SlickScalar, 3, 1> point2, point3;
  for (int i = 0; i < nransac; ++i) {
    ...
    Eigen::Matrix<SlickScalar, 3, 1> v3_mean = 0.33333333 *(points[id1]+point2+point3);
    Eigen::Matrix<SlickScalar, 3, 1> v3_ca = point3 - points[id1];
    Eigen::Matrix<SlickScalar, 3, 1> v3_ba = point2 - points[id1];
    Eigen::Matrix<SlickScalar, 3, 1> v3_normal = v3_ca.cross(v3_ba).normalized();
  ...
  Eigen::Matrix<SlickScalar, 3, 1> plane_point = Vector3d::Zero();
  ...
  Eigen::Matrix<SlickScalar, 3, 1> plane_normal = sym.eigenvectors().col(sm_id);
  ...
  for (unsigned int i = 0; i < inlier_indices.size(); i++) {
    ...
    while (id3 == id2 || id3 == i)
      id3 = rand() % npoints;
    point3 = points[id3];
    Eigen::Matrix<SlickScalar, 3, 1> v3_mean = 0.33333333 *(points[inlier_indices[i]]+point2+point3);
    Eigen::Matrix<SlickScalar, 3, 1> v3_diff = (points[inlier_indices[i]] - v3_mean).normalized();
    SlickScalar distsq = v3_diff.norm();
    if (distsq == 0.0)
      continue;
