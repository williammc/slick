...
#include "slick/math/utilities.h"
...
struct SynthesisPoint {
  SynthesisPoint(){}
  SynthesisPoint(Eigen::Matrix<SlickScalar, 3, 1> c, float radius, cv::Scalar color, float thick)
  ...
  Eigen::Matrix<SlickScalar, 3, 1> center;
  Eigen::Matrix<SlickScalar, 3, 1> top;
  Eigen::Matrix<SlickScalar, 3, 1> bottom;
  Eigen::Matrix<SlickScalar, 3, 1> left;
  Eigen::Matrix<SlickScalar, 3, 1> right;
...
bool check3DPointVisible(const Eigen::Matrix<SlickScalar, 3, 1>& point, KeyframeType* kf) {
...
void generate3DPointsOnPlane(SynthesisModel& model, const int npoints=100,
                             const SlickScalar scale_xyz=1.0) {
  Eigen::Matrix<SlickScalar, 3, 1> point(0,0,1);
  for (int i = 0; i < npoints; ++i) {
    point[0] = SlickScalar(std::rand()%1000-500)/500;
    point[1] = SlickScalar(std::rand()%1000-500)/500;
    SynthesisPoint p(point*scale_xyz, SlickScalar(std::rand()%100)/1000,
                     cv::Scalar(100+std::rand()%155,100+std::rand()%155,100+std::rand()%155),
                     SlickScalar(std::rand()%1000)/200);
...
void generate3DPointsOffPlane(SynthesisModel& model, const int npoints=100,
  ...
  for (int i = 0; i < npoints; ++i) {
    point[0] = SlickScalar(std::rand()%1000-500)/500;
    point[1] = SlickScalar(std::rand()%1000-500)/500;
    point[2] = 0.5 + SlickScalar(std::rand()%1000)/500;
    SynthesisPoint p(point*scale_xyz, SlickScalar(std::rand()%100)/1000,
                     cv::Scalar(100+std::rand()%155,100+std::rand()%155,100+std::rand()%155),
                     SlickScalar(std::rand()%1000)/200);
    model.world_points.push_back(p);
